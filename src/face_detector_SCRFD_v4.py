...
def SCRFD_detector(args):
    ...
    video_tracked = cv2.VideoWriter(
        results_path + suffix, fourcc, fps, (image.shape[1], image.shape[0])
    )

    _root = os.path.expanduser("~/.insightface")
    dir_path = os.path.join(_root, "models", "buffalo_l")
    onnx_file = glob.glob(osp.join(dir_path, "*det_10g.onnx"))
    session = onnxruntime.InferenceSession(
        onnx_file[0],
        providers=["CUDAExecutionProvider", "CPUExecutionProvider"],
        provider_options=None,
    )
    model = SCRFD(onnx_file, session)
    model.prepare(gpu, input_size=det_size, det_thresh=0.5)
...
def model_inf(model, img):
    bboxes, kpss = model.detect(img, max_num=0, metric="default")
    if bboxes.shape[0] == 0:
        return []
    ret = []
